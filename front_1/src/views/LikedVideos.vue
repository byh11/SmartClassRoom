<template>
  <div class="liked-videos-page container">
...
const fetchLikedVideos = async () => {
  ...
  try {
    ...
    const res = await api.getLikedVideos(userid, userType)
    if (res.data.code === 200) {
      ...
      ElMessage.error(res.data.message || '获取喜欢的视频列表失败')
    }
  } catch (error) {
    console.error('获取喜欢的视频列表失败:', error)
    ElMessage.error('获取喜欢的视频列表失败')
  } finally {
    loading.value = false
  }
}
