...
const fetchStudentInfo = async () => {
  try {
    ...
    const response = await api.getStudentInfo(studentid)
...
const handleSave = async () => {
  ...
  await formRef.value.validate(async (valid) => {
    if (valid) {
      try {
        const response = await api.updateStudentInfo(form.studentid, form)
        if (response.data.code === 200) {
...
const handleChangePassword = async () => {
  ...
  await passwordFormRef.value.validate(async (valid) => {
    if (valid) {
      try {
        const response = await api.studentChangePassword(form.studentid, {
...
const beforeAvatarUpload = async (file) => {
  ...
  try {
    const response = await api.updateStudentAvatar(form.studentid, file)
    if (response.data.code === 200) {
